...
QUERY_MONITORING = [
    # OLTP Single Table (query sederhana, 1 tabel) ──
    ("OLTP: Sewa aktif per penyewa",
     "SELECT * FROM sewa WHERE id_penyewa = %s AND tanggal_akhir >= CURRENT_DATE",
     lambda: (run_query("SELECT id_penyewa FROM profil_penyewa LIMIT 1")[0]["id_penyewa"],)),
    ("OLTP: Sewa aktif per kamar",
     "SELECT * FROM sewa WHERE id_kamar = %s AND tanggal_akhir >= CURRENT_DATE",
     lambda: (run_query("SELECT id_kamar FROM kamar LIMIT 1")[0]["id_kamar"],)),
    ("OLTP: Semua sewa masih aktif",
     "SELECT id_sewa, id_kamar FROM sewa WHERE tanggal_akhir >= CURRENT_DATE",
     lambda: ()),
    ("OLTP: Cari penyewa by nama",
    ...
    #OLTP JOIN (query dashboard, banyak tabel) ──
    ("OLTP: Data kamar pemilik (JOIN 4 tabel)",
     q.OLTP_DATA_KAMAR_PEMILIK,
     lambda: ()),
    ("OLTP: Riwayat pembayaran pemilik (JOIN 5 tabel)",
     q.OLTP_RIWAYAT_PEMBAYARAN_PEMILIK,
     lambda: ()),
    ("OLTP: Data penyewa aktif (JOIN 3 tabel)",
     q.OLTP_DATA_PENYEWA_AKTIF,
     lambda: ()),
    ("OLTP: Rekap maintenance (JOIN 3 tabel)",
     q.OLTP_REKAP_MAINTENANCE,
     lambda: ()),
    ("OLTP: Maintenance pending (JOIN 3 tabel)",
     q.OLTP_MAINTENANCE_PENDING,
     lambda: ()),
    ("OLTP: Kamar kosong penyewa (JOIN 3 tabel)",
     q.OLTP_KAMAR_KOSONG_PENYEWA,
     lambda: ()),
    ("OLTP: Daftar penyewa login",
     q.OLTP_DAFTAR_PENYEWA_AKTIF_LOGIN,
     lambda: ()),
    ("OLTP: Sewa aktif penyewa (JOIN 4 tabel)",
     q.OLTP_SEWA_AKTIF_PENYEWA,
     lambda: (run_query("SELECT id_penyewa FROM profil_penyewa WHERE status='Aktif' LIMIT 1")[0]["id_penyewa"],)),
    ("OLTP: Riwayat bayar penyewa (JOIN 4 tabel)",
     q.OLTP_RIWAYAT_BAYAR_PENYEWA,
     lambda: (run_query("SELECT id_penyewa FROM profil_penyewa WHERE status='Aktif' LIMIT 1")[0]["id_penyewa"],)),
    ("OLTP: Maintenance penyewa (JOIN 2 tabel)",
     q.OLTP_MAINT_PENYEWA,
     lambda: (run_query("SELECT id_penyewa FROM request_maintenance LIMIT 1")[0]["id_penyewa"],)),

    #OLAP — Agregasi (COUNT, SUM, AVG, GROUP BY) ──
    ("OLAP: KPI total kamar (COUNT)",
     q.OLAP_KPI_TOTAL_KAMAR,
     lambda: ()),
    ("OLAP: KPI kamar kosong (COUNT + WHERE)",
     q.OLAP_KPI_KAMAR_KOSONG,
     lambda: ()),
    ("OLAP: KPI kamar terisi (COUNT + WHERE)",
     q.OLAP_KPI_KAMAR_TERISI,
     lambda: ()),
    ("OLAP: KPI total pendapatan (SUM)",
     q.OLAP_KPI_TOTAL_PENDAPATAN,
     lambda: ()),
    ("OLAP: KPI maintenance pending (COUNT + JOIN)",
     q.OLAP_KPI_MAINTENANCE_PENDING,
     lambda: ()),
    ("OLAP: Pendapatan per kos (GROUP BY + SUM + AVG)",
     q.OLAP_PENDAPATAN_PER_KOS,
     lambda: ()),
    ("OLAP: Pendapatan per tipe kamar (GROUP BY + SUM)",
     q.OLAP_PENDAPATAN_PER_TIPE_KAMAR,
     lambda: ()),
    ("OLAP: Ringkasan metode bayar (GROUP BY + COUNT + SUM)",
     q.OLAP_RINGKASAN_METODE_BAYAR,
     lambda: ()),
]
